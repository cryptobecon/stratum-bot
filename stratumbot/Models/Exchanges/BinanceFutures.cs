...
namespace stratumbot.Models.Exchanges
{
    ...
    {
        public Exchange Id { get; set; } = Exchange.BinanceFutures;

        public string Name { get; set; } = "Binance Futures";
        ...
        /// </summary>
        public decimal MakerFEE { get; set; } = (decimal)0.02;

        /// <summary>
        /// Taker fee (market order)
        /// </summary>
        public decimal TakerFEE { get; set; } = (decimal)0.04;

        // TODO del
        //public decimal FEE { get; set; } = (decimal)0.0400000; // Комиссия-заглушка
        ...
        public void ClientInit(int tid)
        {
            ...
            {
                Logger.Debug("Not init private client (futures).");
            }
        ...
        {
            return this.OpenOrderAsync(orderSide, cur1, cur2, amount, price, null, /*quote*/ null).Result;
...

        {
            ...
            while (!this.cancellationToken.IsCancellationRequested)
            {
                ...
                var callResult = /*await*/ this.privateClient.UsdFuturesApi.Trading.PlaceOrderAsync
                (
                    _cur1 + _cur2,
                    (_orderSide == DTO.OrderSide.BUY) ? BinanceNet.Enums.OrderSide.Buy : BinanceNet.Enums.OrderSide.Sell,
                    (orderType == "LIMIT") ? FuturesOrderType.Limit : FuturesOrderType.Market,
                    _amount,
                    price: (_price == null) ? (decimal?)null : _price,
                    /*timeInForce: (orderType == "LIMIT") ? TimeInForce.GoodTillCrossing : (TimeInForce?)null,*/
                ...
                else
                {
                    Order order = new Order()
                    {
                        ...
                        Price = callResult.Data.AveragePrice != 0 ? callResult.Data.AveragePrice : callResult.Data.Price,
                        Amount = callResult.Data.Quantity,
                        Filled = callResult.Data.QuantityFilled,
                        Remainder = callResult.Data.Quantity - callResult.Data.QuantityFilled,
                        ...
                        Time = callResult.Data.UpdateTime.ToString(),
                        FeeRate = callResult.Data.Type == FuturesOrderType.Limit ? this.MakerFEE : this.TakerFEE
                    };
        ...
        public async Task<Order> GetOrderInfoAsync(Order _order)
        {
            while (!this.cancellationToken.IsCancellationRequested)
            {
                var callResult = /*await*/ this.privateClient.UsdFuturesApi.Trading.GetOrderAsync
                (
                ...
                else
                {
                    Order order = new Order()
                    {
                        ...
                        Cur2 = _order.Cur2,
                        Id = callResult.Data.Id.ToString(),
                        Price = callResult.Data.AvgPrice != 0 ? callResult.Data.AvgPrice : callResult.Data.Price,
                        Amount = callResult.Data.Quantity,
                        Filled = callResult.Data.LastFilledQuantity,
                        Remainder = callResult.Data.Quantity - callResult.Data.LastFilledQuantity,
                        Side = _order.Side,
                        Status = (callResult.Data.Status == BinanceNet.Enums.OrderStatus.New) ? DTO.OrderStatus.New : (callResult.Data.Status == BinanceNet.Enums.OrderStatus.Filled) ? DTO.OrderStatus.Filled : (callResult.Data.Status == BinanceNet.Enums.OrderStatus.PartiallyFilled) ? DTO.OrderStatus.PartiallyFilled : DTO.OrderStatus.Canceled,
        ...
        public async Task<Order> CancelOrderAsync(Order _order)
        {
            while (!this.cancellationToken.IsCancellationRequested)
            {
                ...
                var callResult = /*await*/ this.privateClient.UsdFuturesApi.Trading.CancelOrderAsync
                (
                    _order.Cur1 + _order.Cur2,
                    orderId: long.Parse(_order.Id),
                    ct: this.cancellationToken
...

        {
            while (!this.cancellationToken.IsCancellationRequested)
            {
                var callResult = /*await*/ this.privateClient.UsdFuturesApi.Account.GetAccountInfoAsync().Result;
                ...
                {
                    return callResult.Data.AvailableBalance;
                }
            }
        ...
        public async Task GetMinimalsAsync(string cur1, string cur2)
        {
            while (!this.cancellationToken.IsCancellationRequested)
            {
                var callResult = await this.publicClient.UsdFuturesApi.ExchangeData.GetExchangeInfoAsync(this.cancellationToken);
                ...
                else
                {
                    ...
                    MinAmount = ((BinanceNet.Objects.Models.Futures.BinanceFuturesSymbol[])callResult.Data.Symbols).First(x => x.Pair == cur1 + cur2).LotSizeFilter.MinQuantity;
                    //MinAmount = ((BinanceNet.Objects.Models.Futures.BinanceFuturesSymbol[])callResult.Data.Symbols)[0].LotSizeFilter.MinQuantity;
                    // Amount step size (ex: +-0.0001 ETH)
                    StepSize = ((BinanceNet.Objects.Models.Futures.BinanceFuturesSymbol[])callResult.Data.Symbols).First(x => x.Pair == cur1 + cur2).LotSizeFilter.StepSize;
                    // Minimal total cost of second currency (ex: 10 USDT)
                    MinCost = ((BinanceNet.Objects.Models.Futures.BinanceFuturesSymbol[])callResult.Data.Symbols).First(x => x.Pair == cur1 + cur2).MinNotionalFilter.MinNotional;
                    // Price tick size (ex: +0.01 USDT)
                    TickSize = ((BinanceNet.Objects.Models.Futures.BinanceFuturesSymbol[])callResult.Data.Symbols).First(x => x.Pair == cur1 + cur2).PriceFilter.TickSize;
                    break;
                }
            }
        }
        ...
        public Dictionary<string, List<Depth>> GetDOM(string cur1, string cur2, int limit = 5)
        {
            while (!this.cancellationToken.IsCancellationRequested)
            {
                var prices = this.publicClient.UsdFuturesApi.ExchangeData.GetOrderBookAsync(cur1 + cur2, this.GetDepthLimit(limit), this.cancellationToken).Result;
...

        {
            while (!this.cancellationToken.IsCancellationRequested)
            {
                var result = /*await*/ this.publicClient.UsdFuturesApi.ExchangeData.GetKlinesAsync
                (
                    cur1 + cur2,
        ...
        public decimal PriceChange(string cur1, string cur2)
        {
            ...
            return this.publicClient.UsdFuturesApi.ExchangeData.GetTickerAsync(cur1 + cur2, this.cancellationToken).Result.Data.PriceChangePercent;
        ...
        {
            // TODO add async + while (!this.cancellationToken.IsCancellationRequested)
            return this.publicClient.UsdFuturesApi.ExchangeData.GetTickerAsync(cur1 + cur2).Result.Data.Volume;
        }
        ...
        private void ErrorHandler(CryptoExchange.Net.Objects.Error error)
        {
            try
            {
                Logger.ToFile(error.ToString(), "[error] BinanceFutures.ErrorHandler");
				...
				else if (error.Message.Contains("Margin is insufficient"))
				{
					Logger.Error(_.Log62);
				}
				else if (error.Message.Contains("Order's notional must be no smaller than"))
				{
					Logger.Error("code 35");
					// TODO automatocally
					throw new ManuallyStopException("Остановка изза MIN_NOTIONAL");
            ...
            {
                Logger.ToFile($"[exception] BinanceFutures.ErrorHandler {ex.ToString()}");
            }
        }
    }
}
