...
namespace stratumbot.Models.Filters
{
    class CCI : IFilter
    {
        ...
        public string MyName { get; set; } = "CCI"; // Отображаемое название 
        ...
        // Конструктор
        public CCI(int moreOrLess, decimal priceValue, int period, string timeFrame, int duration)
        {
        ...
        public void Compute()
        {
            ...
            //var cci = new CCIi();
            //var ccindex = cci.Result(this.DataMiner.Klines[this.TimeFrame], this.Period);

            IEnumerable<CciResult> results =
                this.DataProvider.Quotes.GetCci(this.Period);

            var cciIndex = results.Last().Cci;

            //Logs.Logger.Info("CCI index: " + results.Last().Cci.ToString());

            if (Mode == 0) // "cci<value"
            {
                Logs.Logger.ToFile($" /{this.ID}/{this.Mode}/ ?: {results.Last().Cci} < {this.PriceValue}");
                if (results.Last().Cci < this.PriceValue)
                {
                    this.Result = true;
