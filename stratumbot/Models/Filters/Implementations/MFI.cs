...
namespace stratumbot.Models.Filters
{
    class MFI : IFilter
    {
        ...
        // Настройки
        public int Mode = 0; // 0 - Less(mfi<value); 1 -  More(mfi>value)
        ...
        // Конструктор
        public MFI(int moreOrLess, decimal priceValue, int period, string timeFrame, int duration)
        {
        ...
        public void Compute()
        {
            ...
            //var mfi = new MFIi();
            //var data = mfi.Result(this.DataMiner.Klines[this.TimeFrame], this.Period);

            IEnumerable<MfiResult> results = this.DataProvider.Quotes.GetMfi(this.Period);

            //Logs.Logger.Info(results.Last().Mfi.ToString());

            if (Mode == 0) // "mfi<value"
            {
                Logs.Logger.ToFile($" /{this.ID}/{this.Mode}/ ?: {results.Last().Mfi} < {this.PriceValue}");
                if (results.Last().Mfi < this.PriceValue)
                {
                    this.Result = true;
