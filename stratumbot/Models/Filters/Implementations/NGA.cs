...
namespace stratumbot.Models.Filters
{
    class NGA : IFilter
    {
        ...
        public string MyName { get; set; } = "NGA"; // Отображаемое название 
        ...
        // Конструктор
        public NGA(int higherOrlower, int period, string timeFrame, decimal indent, int duration, string depthSide)
        {
        ...
        public void Compute()
        {
            ...
            var nga = new NGAi();
            var avg = nga.Result(this.DataProvider.Quotes, this.Period);

            // Высчитываем отступ
            decimal currentPoint = avg;
            currentPoint = avg + Calc.AmountOfPercent(this.Indent, currentPoint);
