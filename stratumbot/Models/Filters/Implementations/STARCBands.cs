...
{
	internal class STARCBands : IFilter
	{
		/// <summary>
		/// ID
		/// </summary>
		...
		/// Name of the filter
		/// </summary>
		public string Name { get; set; } = "STARC Bands";
		...
		/// </summary>
		public string MyName { get; set; } = "STARC Bands";
		...
		// Конструктор
		public STARCBands(int moreOrLess, int period, int period2, decimal rate, string timeFrame, decimal indent, int duration)
		{
		...
		public DataOptions GetOptions(DataType dataType, string cur1 = null, string cur2 = null)
		{
			if (dataType == DataType.Quotes)
			{
				return new DataOptions
				{
					...
					Cur2 = cur2,
					TimeFrame = this.TimeFrame,
					Period = Math.Max(this.Period, this.Period2 + 100)
				};
			}
		...
		public void Compute()
		{
			...
			IEnumerable<StarcBandsResult> results =
				this.DataProvider.Quotes.GetStarcBands(this.Period, this.Rate, this.Period2);
