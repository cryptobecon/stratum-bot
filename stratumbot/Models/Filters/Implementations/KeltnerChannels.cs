...
{
	internal class KeltnerChannels : IFilter
	{
		/// <summary>
		/// ID
		/// </summary>
		public string ID { get; set; } = "25";

		/// <summary>
		/// Name of the filter
		/// </summary>
		public string Name { get; set; } = "Keltner Channels";
		...
		/// </summary>
		public string MyName { get; set; } = "Keltner";
		...
		// Конструктор
		public KeltnerChannels(int moreOrLess, int period, int period2, decimal rate, string timeFrame, decimal indent, int duration)
		{
		...
		public DataOptions GetOptions(DataType dataType, string cur1 = null, string cur2 = null)
		{
			...
			{
				int N = Math.Max(this.Period, this.Period2);
				return new DataOptions
				{
					Cur1 = cur1,
					Cur2 = cur2,
					TimeFrame = this.TimeFrame,
					Period = Math.Max(N * 2, N + 100)
				};
			}
		...
		public void Compute()
		{
			...
			IEnumerable<KeltnerResult> results =
				this.DataProvider.Quotes.GetKeltner(this.Period, this.Rate, this.Period2);
